...
- **TCP_NODELAY Enforcement**: Explicitly disables Nagle’s Algorithm at the socket level. This prevents the network stack from waiting to "fill up" packets, ensuring that even the smallest data (such as attack packets or movement updates) are transmitted instantly.
...
2. **Socket Tuning**: During the connection initialization phase (specifically in `addHandlers`), the mod injects a socket configuration call that sets `ChannelOption.TCP_NODELAY` to `true`. This is a low-level instruction to the OS to bypass Nagle's algorithm.
3. **Thread Safety**: All flushing operations are performed within the appropriate Netty event loop or validated against the channel state to ensure no race conditions or synchronization overhead.
